...
- The dense guard's carve-out windows and their `(0.15, 1.45)` budgets are
  ...
  two-sidedly (the port may snap where the web snaps, and must not smooth a
  snap away).
- Ranking 13 (the ~9 cm pole-grip shortfall) closed as a consequence: the
  chain now closes on the point the web closes on. Ranking 14 (window 2's
  spin crossing phase) stays open and is pinned as a record rather than a
  budget.
