...
pub struct DetailHeader {
    ...
    /// Interval piece (the table title changes).
...
/// Column header ids for the splits table (web `replay.th*`; the power
/// column reuses `replay.cPower` — the web table's `thWattMin` is
/// watt-minutes, which the strip already covers).
#[must_use]
