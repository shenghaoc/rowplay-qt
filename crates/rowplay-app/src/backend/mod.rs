...
/// like a real sync. (`ROWPLAY_DATA_DIR`, `ROWPLAY_FORCE_COLOR_SCHEME`,
/// `ROWPLAY_FORCE_CONTRAST` and `ROWPLAY_EXIT_AFTER_FRAMES` are deliberately
/// *not* here: those are documented user-facing overrides, and the last one
/// is what the packaged release bundles are launched with to prove they
/// start — see `tools/package/`.)
