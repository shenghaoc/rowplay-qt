...
// one replace() each. The charts sit in one card (HIG Charts, ADR 0013).
import QtQuick
...
ColumnLayout {
    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            // Rust). Studio titles this panel "Split Focus", which has no web
            // key; the web heads its per-stroke inspector section with
            // inspector.sectionPerStroke, so the charts no longer repeat the
            // splits table's heading below them.
            ColumnLayout {
                Layout.fillWidth: true
                spacing: Theme.spacingXSmall
            ...
            // boundaries — Studio's paceChart; the ticks are the
            // Rust-formatted pace labels, never the negated seconds.
            StrokeChart {
                id: paceChart
                Layout.fillWidth: true
            ...
            // later segments are summarised by the count, not drawn (4b
