...
// Vector glyphs drawn in QML (ADR 0013): SF Symbols are Apple-licensed and
// Apple-only, and nothing may be added under assets/ (the tree is hash
// pinned), so the shell's symbols are authored here as SVG path data on a
// 16-unit grid — one 1.5-unit stroke with round caps and joins, plus an
// optional fill for solid parts — in the style of SF Symbols' regular
// weight. Every path is original to this repository.
//
// Colour comes from the caller (`color`), exactly like a template image, so
// a glyph follows its control's state (accent when checked, white on a
// focused selection, disabled opacity from the parent).
import QtQuick
