...
- `replay-venue-shadow-parity.json` (issue #96) records which venue meshes
  ...
  flags are read off the scene the web's production renderer builds:
  `CourseRenderer3D`, constructed headless as the web's own
  `renderer3d.test.ts` constructs it (a stand-in `WebGLRenderer`, stub
  `document` and `window`), so the environment builder's objects and the
  renderer's own infield, apron and vertical arcs carry whatever flags the
  web leaves on them; one rendered frame must not change any. The venue's
  roots are then named as the bake names the GLBs' nodes. 464 meshes over
  six variants. Regenerate with
